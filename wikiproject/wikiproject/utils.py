    ...
    if _API_KEY is None:
        ...
        try:
            with open('.open_webui_api_key', 'r') as f:
                open_webui_api_key = f.read().strip()
            if _API_KEY is None:
...
def upload_files(fileapth, knowledge_id):
    pass

def update_file_content(filepah, file_id):
    pass
...
def upload_or_update_files(df, remote_files):
    ...
    for idx, row in df.iterrows():
        ...
        if filename not in remote_files:
            ...
            print(f"Updating existing file: {filename}({file_id}) in {knowledge}({knowledge_id})")
